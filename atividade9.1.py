...
class Rede(tk.Frame):
	...
	def criar_Rede(self):
		...
		self.criar_no = tk.Button(self.canvas, text="Nó", width="10", command=self.criar_No)
		self.criar_no.place(relx=.2, rely=0.96, anchor="w")

		self.salvar_topologia = tk.Button(self.canvas, text="Salvar topologia", width="15", command=self.salvar_Topologia)
		self.salvar_topologia.place(relx=.6, rely=.96, anchor="e")

		self.criar_aresta = tk.Button(self.canvas, text="Aresta", width="10", command=self.criar_Aresta)
		...
		for i in range(len(self.grafo)):
			for j in range(len(self.grafo)):
				if (self.grafo[i][j]!=0 and i<=j):
					novaAresta = Aresta(self.canvas,self.grafo[i][j], self.listadeNos, i, j)
